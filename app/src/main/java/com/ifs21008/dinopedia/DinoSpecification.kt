...
import com.ifs21008.dinopedia.databinding.ActivityDinoSpecificationBinding

class DinoSpecification : AppCompatActivity() {

    private lateinit var binding: ActivityDinoSpecificationBinding
    private var dino: Dinosaurus? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityDinoSpecificationBinding.inflate(layoutInflater)
        setContentView(binding.root)

        dino = if (Build.VERSION.SDK_INT >=33) {
            intent.getParcelableExtra(EXTRA_ITEM, Dinosaurus::class.java)
        } else {
